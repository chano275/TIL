...
# 예시 데이터 생성
# y = 2 * x + 3 의 선형 회귀 모델을 학습시키기 위한 데이터
...
# 모델 초기화: 선형 모델 y = wx + b
# w와 b는 학습을 통해 모델이 찾으려는 가중치와 바이어스입니다.
...
# 학습률 설정
# 학습률은 파라미터를 얼마나 크게 업데이트할지를 결정합니다.
learning_rate = 0.01

# SGD 구현
# 총 100번의 epoch 동안 모델을 학습시킵니다.
for epoch in range(100):
    # 순전파 계산 (예측값 계산)
    # 모델의 예측값은 y_pred = wx + b 로 계산됩니다.
    y_pred = x_data * w + b

    # 손실 함수 (MSE)
    # 예측값과 실제 값 사이의 차이를 평균 제곱 오차(MSE)를 통해 계산합니다.
    loss = ((y_pred - y_data) ** 2).mean()  # 손실 값 계산

    # 역전파: 기울기 계산
    # 역전파 단계에서 손실에 대한 w와 b의 기울기를 계산합니다.
    loss.backward()  # 자동으로 w.grad와 b.grad에 기울기가 저장됩니다.

    # 파라미터 업데이트 (SGD 적용)
    # 손실 함수의 기울기를 사용해 파라미터(w, b)를 업데이트합니다.
    # torch.no_grad()는 기울기 추적을 중지시키는 컨텍스트로, 파라미터 업데이트 시 필요합니다.
    with torch.no_grad():
        w -= learning_rate * w.grad  # w = w - learning_rate * w.grad
        b -= learning_rate * b.grad  # b = b - learning_rate * b.grad
