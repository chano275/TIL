...
print("\n문제 2. <title> 태그 내용 추출")
print("답: 웹페이지 제목:", title.text if title else "제목 없음")
print("설명: BeautifulSoup을 사용하여 HTML에서 <title> 태그의 내용을 추출했습니다.")

# 3. <a> 태그 개수 세기
links = soup.find_all('a')
print("\n문제 3. <a> 태그 개수 세기")
print("답: 링크 개수:", len(links))
print("설명: find_all() 메소드를 사용하여 모든 <a> 태그를 찾고, 그 개수를 계산했습니다.")

# 4. 외부 링크 필터링 후 첫 5개의 href 속성 출력
external_links = [link.get('href') for link in links if link.get('href') and ('http' in link.get('href') or 'https' in link.get('href'))]
print("\n문제 4. 외부 링크 필터링 후 첫 5개의 href 속성 출력")
print("답:", external_links[:5])
print("설명: href 속성에 'http' 또는 'https'가 포함된 외부 링크를 필터링하여 출력했습니다.")

# 5. 'traffic' 또는 '교통'이 포함된 텍스트 찾기
traffic_texts = soup.find_all(string=lambda string: any(keyword in string.lower() for keyword in ['traffic', '교통']) if string else False)
filtered_texts = [text.strip() for text in traffic_texts if text.parent.name in ['p', 'div', 'span', 'li', 'strong', 'a']]
