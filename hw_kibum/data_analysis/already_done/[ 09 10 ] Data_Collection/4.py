...
response = requests.get(url, verify=False)  # SSL 인증서 검증을 비활성화하여 데이터를 가져옵니다.
...
# 4. 모든 <td> 태그 내용 추출
td_contents = [td.text.strip() for td in first_table.find_all('td') if td.text.strip()]
print("\n문제 4. 모든 <td> 태그 내용 추출")
print("답: <td> 태그 내용:")
...
if td_contents:
    ...
    print("답: 정제된 데이터:")
    for i, road_type in enumerate([general_road, city_highway]):
        print(f"{road_types[i]} 기준:")
        for j, speed in enumerate(road_type):
            if j < len(speed_meanings):
                print(f"  - {speed_meanings[j]}: {speed}")
            else:
                print(f"  - 추가 정보: {speed}")
...
print("\n주의: 이 예제에서는 SSL 인증서 검증을 비활성화했습니다. 실제 환경에서는 보안을 위해 이 방식을 사용하지 않는 것이 좋습니다.")