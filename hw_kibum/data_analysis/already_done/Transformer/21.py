...
class PositionalEncoding(nn.Module):
    def __init__(self, embedding_dim, dropout_p=0.1, max_len=512):
        ...
        self.dropout = nn.Dropout(p=dropout_p)        # Dropout 레이어로 과적합 방지

        # 위치 인코딩을 위한 사인 및 코사인 계산
        pos_encoding = torch.zeros(max_len, embedding_dim)        # pos_encoding의 역할: 각 단어 임베딩에 위치 정보를 더해주는 역할
        positions_list = torch.arange(0, max_len, dtype=torch.float).unsqueeze(1)        # 위치 값 (문장 내에서 단어의 위치)
        division_term = torch.exp(torch.arange(0, embedding_dim, 2).float() * (-math.log(10000.0)) / embedding_dim)        # 주기적인 패턴을 추가하기 위한 scaling factor 계산
        # TODO: 짝수 인덱스에는 사인 함수를 홀수 인덱스에는 코사인 함수를 적용하세요.
        pos_encoding[:, 0::2] = torch.sin(positions_list * division_term)        # 짝수 인덱스에 사인 함수를 적용하여 위치 인코딩 값을 계산
        pos_encoding[:, 1::2] = torch.cos(positions_list * division_term)        # 홀수 인덱스에 코사인 함수를 적용하여 위치 인코딩 값을 계산

        pos_encoding = pos_encoding.unsqueeze(0)        # 배치 차원을 추가하여 각 시퀀스의 위치 인코딩을 독립적으로 적용할 수 있도록 합니다.
        self.register_buffer('pos_encoding', pos_encoding)        # 학습되지 않는 파라미터로 등록

    def forward(self, token_embedding):
        seq_len = token_embedding.size(1)        # 입력 시퀀스 길이에 맞춰 위치 인코딩 값을 잘라 적용합니다.
        pos_encoding = self.pos_encoding[:, :seq_len, :]
        return self.dropout(token_embedding + pos_encoding)        # 입력 임베딩에 위치 인코딩을 더한 후 Dropout을 적용하여 과적합을 방지합니다.


# --------------------- Transformer Encoder Layer ---------------------
'''
Transformer 모델의 인코더 레이어는 Self-Attention과 Feed-Forward 신경망을 통해 입력 문장 내 각 단어의 상호 관계를 학습합니다.
'''
...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, embedding_dim, num_heads, hidden_dim, dropout_p=0.1):
        ...
        self.self_attention = nn.MultiheadAttention(embedding_dim, num_heads, dropout=dropout_p)        # Multihead Self-Attention 레이어. 각 단어 간의 관계를 학습합니다.
        self.feed_forward = nn.Sequential(        # Feed-Forward Network 레이어. 각 단어의 의미를 더욱 정교하게 학습합니다.
        ...
        self.layer_norm1 = nn.LayerNorm(embedding_dim)        # Layer Normalization 레이어. 입력값을 정규화하여 학습의 안정성을 높입니다.
        self.layer_norm2 = nn.LayerNorm(embedding_dim)
        self.dropout = nn.Dropout(dropout_p)        # Dropout 레이어. 과적합을 방지합니다.
        self.scale = math.sqrt(embedding_dim)        # Self-Attention 값에 대한 스케일링을 위한 상수입니다.

    def forward(self, x):
        # TODO: Layer Normalization을 적용한 Self-Attention을 수행하세요.
        x_norm = self.layer_norm1(x)
        attn_output, _ = self.self_attention(x_norm, x_norm, x_norm)
        x = x + self.dropout(attn_output)  # Residual Connection을 사용하여 Self-Attention 출력과 이전 층의 출력을 더합니다. 이는 학습 안정성을 높이고 기울기 소실 문제를 완화합니다.

        # TODO: Layer Normalization을 적용한 후, Feed-Forward Network를 적용하세요.
        x_norm = self.layer_norm2(x)
        ff_output = self.feed_forward(x_norm)
        x = x + self.dropout(ff_output)        # Residual Connection을 사용하여 Feed-Forward Network의 출력과 이전 층의 출력을 더합니다. 이는 학습 안정성을 높이고 기울기 소실 문제를 완화합니다.
...
class TransformerDecoderLayer(nn.Module):
    def __init__(self, embedding_dim, num_heads, hidden_dim, dropout_p=0.1):
        ...
        self.self_attention = nn.MultiheadAttention(embedding_dim, num_heads, dropout=dropout_p)        # 디코더 Self-Attention: 타겟 시퀀스의 각 단어 간의 관계를 학습한다.
        self.enc_dec_attention = nn.MultiheadAttention(embedding_dim, num_heads, dropout=dropout_p)        # Cross-Attention: 인코더의 출력과 디코더의 타겟 시퀀스 간의 관계를 학습한다.
        self.feed_forward = nn.Sequential(        # Feed-Forward Network. 타겟 시퀀스의 각 단어를 정교하게 학습한다.
        ...
        self.layer_norm1 = nn.LayerNorm(embedding_dim)        # Layer Normalization 레이어. 학습의 안정성을 높이기 위해 입력을 정규화합니다.
        self.layer_norm2 = nn.LayerNorm(embedding_dim)
        self.layer_norm3 = nn.LayerNorm(embedding_dim)
        self.dropout = nn.Dropout(dropout_p)        # Dropout 레이어. 과적합을 방지합니다.
        self.scale = math.sqrt(embedding_dim)

    def forward(self, x, memory, tgt_mask=None):  # 디코더 Self-Attention 수행. 타겟 시퀀스의 각 단어 간 관계를 학습
