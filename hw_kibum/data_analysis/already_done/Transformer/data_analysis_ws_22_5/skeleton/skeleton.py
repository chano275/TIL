...
# 8. Trainer 초기화
# Trainer는 모델 훈련과 평가를 간편하게 할 수 있도록 도와주는 transformers에서 제공하는 객체입니다.
# 이전에 구현해놓은 코드를 기반으로 값을 채워주세요.
# 참고: https://huggingface.co/docs/transformers/main_classes/trainer
trainer = _____________(
    model=_____________,                         # 훈련할 모델
    args=_____________,                  # 훈련 인자 설정
    train_dataset=_____________,         # 훈련 데이터셋
    eval_dataset=_____________,           # 평가 데이터셋
    tokenizer=_____________,                 # 토크나이저 설정 (훈련 과정에서 필요)
    compute_metrics=_____________      # 매트릭 설정
)

# 9. 모델 학습
# 정의한 Trainer를 사용하여 모델을 실제로 훈련시킵니다.
# 참고: https://huggingface.co/docs/transformers/main_classes/trainer#transformers.Trainer.train
trainer.train()

# 10. 모델 평가
# 훈련된 모델을 테스트 데이터셋으로 평가합니다.
# 참고: https://huggingface.co/docs/transformers/main_classes/trainer#transformers.Trainer.evaluate
eval_results = trainer.evaluate()

# eval_results에 뭐가 들어오는지 확인하기 위해서 출력
# 평가 결과 전체를 출력하여 어떤 메트릭들이 계산되었는지 확인합니다.
