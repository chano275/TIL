...
# 간단한 학습 데이터셋
# 영화 리뷰 문장과 긍정(1) 또는 부정(0) 레이블을 함께 포함하고 있습니다.
train_data = [
    ("I love this movie", 1),
...
# 간단한 테스트 데이터셋
# 학습된 모델이 테스트 데이터에서 얼마나 잘 동작하는지 평가합니다.
test_data = [
...
# 텍스트를 숫자로 변환하는 함수
# 각 단어를 어휘집에 따라 숫자로 바꿉니다. 어휘집에 없는 단어는 0으로 처리합니다.
# 예: "I love this movie" -> [1, 2, 3, 4]
def text_pipeline(sentence):
    return [vocab.get(word, 0) for word in tokenize(sentence)]

# 학습 데이터와 테스트 데이터를 숫자로 변환한 후, 텐서로 만듭니다.
# 텐서는 파이토치에서 데이터를 처리하는 기본 형식입니다.
...
class RNN(nn.Module):
    def __init__(self, input_dim, embedding_dim, hidden_dim, output_dim):
        super().__init__()
        # 단어를 고정된 크기의 벡터로 변환하는 임베딩 레이어입니다.
        self.embedding = nn.Embedding(input_dim, embedding_dim)
        # RNN 레이어: 단어 벡터를 순차적으로 입력받아 학습합니다.
        # 참고: https://pytorch.org/docs/stable/generated/torch.nn.RNN.html
        self.rnn = nn.RNN(embedding_dim, hidden_dim, batch_first=True)
        # 완전 연결 레이어: RNN의 출력에서 최종 예측을 수행합니다.
        self.fc = nn.Linear(hidden_dim, output_dim)

    def forward(self, text):
        # 입력된 텍스트를 임베딩 벡터로 변환합니다.
        embedded = self.embedding(text)
        # RNN을 사용해 임베딩된 단어들을 처리합니다.
        output, hidden = self.rnn(embedded)
        # RNN의 출력값을 완전 연결 레이어를 통해 최종 예측값으로 변환합니다.
...
def train(model, texts, labels, optimizer, criterion):
    ...
    for text, label in zip(texts, labels):
        ...
        loss = criterion(predictions, label.long().unsqueeze(0))
