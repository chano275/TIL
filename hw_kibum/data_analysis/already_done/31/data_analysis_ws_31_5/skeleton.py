...
# 4. LLM의 성능 분석 1: 기본 메트릭을 통한 평가
# MLflow에서 기본적으로 제공하는 다양한 메트릭을 사용하여 모델 성능을 평가합니다.

    # 참고 페이지: https://mlflow.org/docs/latest/python_api/mlflow.html#mlflow.evaluate
    # - `model_uri`: 평가할 모델의 URI입니다.
    # - `data`: 모델에 제공할 입력 데이터 또는 예시입니다.
    # - `targets`: 예측 결과와 비교할 기준 정답 데이터입니다.
    # - `model_type`: 평가할 모델의 유형입니다.
    # - `evaluators`: 사용할 평가 지표입니다.
    # TODO: 해당 빈칸에 필요한 값을 넣어주세요.
    results_default = mlflow.______________(
        traffic_qa_model.model_uri,
        eval_df,
        targets=______________, # 해당 값을 위해 정답에 해당되는 것을 targets으로 넣어주세요. (이전에 question과 ground_truth를 만들었습니다.)
        ______________="question-answering",
        ______________="default",
    )
    print("기본 메트릭을 사용한 평가가 완료되었습니다.")
    print(results_default.metrics)
    print("\n")

    #########################################################################################################

# 5. LLM의 성능 분석 2: 사용자 정의 메트릭을 통한 평가
    
    #  `extra_metrics`: 기본 메트릭에 더해 추가로 평가할 사용자 정의 메트릭의 리스트입니다. 
    #   이전에 만들었던 `answer_similarity_metric`와 `professionalism_metric` 같은 커스텀 메트릭을 추가하여,
    #   모델이 사용자 정의 평가 기준에 얼마나 적합한지 분석할 수 있습니다.
    # TODO: 해당 빈칸에 필요한 값을 넣어주세요.
    results_custom = mlflow.______________(
        traffic_qa_model.model_uri,
        eval_df,
        targets=______________,
        ______________="question-answering",
        ______________="default",
        extra_metrics=[______________, ______________],
    )
    print("사용자 정의 메트릭을 활용한 평가가 완료되었습니다.")
    print(results_custom.metrics)
