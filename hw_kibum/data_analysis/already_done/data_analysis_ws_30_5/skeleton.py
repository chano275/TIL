...
# HuggingFace의 TRL SFTTrainer를 사용하여 모델을 훈련합니다. 
# 이 트레이너는 훈련 루프를 간소화하고 Transformers 라이브러리와 원활하게 통합됩니다.

# 참고 :https://huggingface.co/docs/trl/sft_trainer

from trl import SFTTrainer
from transformers import TrainingArguments

# 토크나이저의 패딩을 오른쪽으로 설정합니다.
# TODO: padding_side의 위치에 해당되는 빈칸을 채워주세요.
# https://huggingface.co/docs/transformers/main_classes/tokenizer
tokenizer.padding_side = _______  # 패딩을 입력 시퀀스의 오른쪽에 추가하도록 설정합니다.

# SFTTrainer 초기화
# TODO: Trainer의 빈칸을 채워주세요.
# trainer를 만들기 위해서는 이전에 설명한 SFTTrainer를 사용해야 합니다.
trainer = _______________(
    model=model,                       # 훈련할 모델
    tokenizer=tokenizer,               # 토크나이저
    train_dataset=dataset,             # 훈련 데이터셋
    eval_dataset=dataset,              # 평가 데이터셋 (여기서는 동일한 데이터셋 사용)
    dataset_text_field="text",         # 텍스트 데이터가 있는 필드
    max_seq_length=max_seq_length,     # 최대 시퀀스 길이
    dataset_num_proc=2,                # 데이터 처리에 사용할 프로세스 수
    packing=False,                     # 짧은 시퀀스에 대한 훈련 속도를 높이려면 True로 설정
    # args에는 TrainingArguments 객체를 활용해서 전달해야 합니다.
    args=_______________(
        per_device_train_batch_size=2,       # 디바이스당 훈련 배치 크기
        gradient_accumulation_steps=4,       # 그래디언트 누적 단계 수
        warmup_steps=5,                      # 워밍업 스텝 수
        num_train_epochs=3,                  # 훈련 에폭 수
        max_steps=30,                       # 최대 훈련 스텝 수
        do_eval=True,                        # 훈련 중 평가 활성화
        evaluation_strategy="steps",         # 평가 전략
        logging_steps=1,                     # 로깅 빈도
        learning_rate=2e-4,                  # 학습률
        fp16=not torch.cuda.is_bf16_supported(),  # bf16이 지원되지 않으면 fp16 사용
        bf16=torch.cuda.is_bf16_supported(),      # bf16이 지원되면 bf16 사용
...
# TODO: 통계 스탯을 볼 수 있도록 SFTTrainer을 학습시킵니다.
# 모델을 훈련시키고 통계를 반환합니다. SFTTrainer를 활용한 trainer 객체는 기본 train방식을 사용해서 통계 수치를 제공합니다.
trainer_stats = trainer._________()  


####################################### Inference #######################################

### 추론
# 훈련 후, 모델을 사용하여 추론을 수행할 수 있습니다. 추론을 위해 모델을 최적화하고, 생성을 제어하기 위해 사용자 정의 정지 기준을 사용합니다.
...
- tokenizer를 사용하여 특정 포맷의 프롬프트를 토큰화하고, 이를 CUDA 기반의 텐서로 변환합니다. 
- 이 과정에서 피보나치 수열을 계속하는 지시문, 입력값, 그리고 출력값을 위한 빈 공간을 포함합니다.
- TextStreamer 객체를 tokenizer와 함께 초기화하여 텍스트 스트리밍 기능을 활성화합니다.
- model.generate 함수를 호출하여 주어진 입력에 대한 텍스트 생성을 수행합니다. 
- 이때, 최대 128개의 새로운 토큰을 생성할 수 있도록 설정하고, TextStreamer를 사용하여 결과를 스트리밍합니다.
"""
...
# TODO: "자율주행에 대한 한국의 계획은?"에 해당하는 질문을 인퍼런스 할 수 있도록 코드를 구현해서 위와 같은 형태로 스트림형태로 출력되도록 구현하세요.
# 이전에 응답을 받은 코드와 동일한 구성을 통해 다른 질의에 대한 답변을 얻을 수 있도록 하면 됩니다.
"""
# 여기에 "자율주행에 대한 한국의 계획은?"에 대한 질문을 언퍼런스 할 수 있도록 코드를 구현하세요.
"""


model.save_pretrained("Llama1b-tuning")  # 모델을 로컬에 저장합니다.

# 정리
"""
CUDA 장치 호환성 확인: 하드웨어가 사용하려는 작업과 라이브러리에 적합한지 확인합니다.
필요한 라이브러리 설치: 종속성을 관리하여 충돌을 방지하고 원활한 작동을 보장합니다.
사전 훈련된 모델 로드 및 구성: unsloth를 사용하여 효율적으로 모델을 로드하고 구성합니다.
LoRA 어댑터 적용: 모델 파라미터의 일부만 업데이트하여 효율적으로 미세 조정합니다.
데이터셋 준비: 효과적인 훈련을 위해 데이터를 올바르게 포맷하고 처리합니다.
모델 훈련: SFTTrainer를 활용하여 적절한 자원 관리와 함께 훈련을 간소화합니다.
추론 수행: 최적화된 설정과 사용자 정의 정지 기준으로 프롬프트에 대한 응답을 생성합니다.
모델 저장: 미세 조정된 모델을 저장하여 향후에 사용할 수 있도록 합니다.

더 많은 정보를 방법론을 적용하고 싶으면 unsloth 공식 페이지: https://docs.unsloth.ai/ 를 참고하면 다양한 방법을 적용할 수 있습니다.
