...
def set_seed(seed):
    ...
    torch.backends.cudnn.deterministic = True  # CUDA에서 결정론적 연산 보장
    torch.backends.cudnn.benchmark = False  # 성능 향상 위한 비결정적 최적화 비활성화
...
def load_dataset_and_preprocess(file_path='traffic_data.csv', model_name='skt/kogpt2-base-v2', max_length=128):
    ...
    """
    tokenizer = load_tokenizer(model_name=model_name)  # TODO: 이전에 만든 토크나이저 로드
    data = load_dataset('csv', data_files=file_path, split='train')  # TODO: datasets.load_dataset를 통해 train데이터로 로드
    ...
    tokenized_dataset = data.map(tokenize_function, batched=True)  # 데이터셋을 토큰화
...
def create_dataloader(dataset, batch_size=2, shuffle=True):
    ...
    """
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=shuffle)  # TODO: 데이터로더 생성을 위해 받아온 인자값을 통해 dataloader를 만들어주세요.
    return dataloader
...
def load_model(model_name='skt/kogpt2-base-v2', tokenizer=None):
    ...
    if tokenizer is not None and tokenizer.pad_token is not None:
        model.resize_token_embeddings(len(tokenizer)) # TODO: token embedding을 추가된 tokenizer를 위해 resize해줍니다.
    
    return model

# 5. 모델 미세 조정(Fine-tuning)
def train_model(model, dataloader, epochs=10, learning_rate=5e-5, device='cuda' if torch.cuda.is_available() else 'cpu'):
    """
...
def chat_with_model(model, tokenizer, max_length=100, device='cuda' if torch.cuda.is_available() else 'cpu'):
    ...
    while True:  # 사용자와 대화 반복
        ...
        input_ids = tokenizer.encode(user_input, return_tensors='pt').to(device)  # 사용자 입력 토큰화 및 장치 이동
...
def main():
    ...
    chat_with_model(model, tokenizer)  # 챗봇 시작
