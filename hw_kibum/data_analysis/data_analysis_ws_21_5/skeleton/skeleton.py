...
# 시드 고정 함수: 실험의 재현성을 보장하기 위해 모든 난수 생성기의 시드를 고정
# 참고: https://pytorch.org/docs/stable/notes/randomness.html
def set_seed(seed):
    random.seed(seed)
    np.random.seed(seed)
...
# 시드 고정 (시드 값을 42로 설정하여 실험 결과가 일관되게 함)
set_seed(42)

# Transformer 모델을 생성하여 GPU로 이동
# TODO: 만든 transformer 모델을 기반으로 
# vocab_size, embedding_dim=128, num_heads=4, hidden_dim=256, num_encoder_layers=6, num_decoder_layers=6, output_dim=2, dropout_p=0.1로 채워넣고. device에 모델을 올려주세요.
model = TransformerModel(vocab_size=vocab_size).to(device)

# 손실 함수: 다중 클래스 분류를 위한 교차 엔트로피 손실 함수 사용
# 참고: https://pytorch.org/docs/stable/generated/torch.nn.CrossEntropyLoss.html
criterion = nn.CrossEntropyLoss()

# 옵티마이저: Adam 옵티마이저 사용 (학습률 0.0001로 설정)
# 참고: https://pytorch.org/docs/stable/optim.html#torch.optim.Adam
optimizer = torch.optim.Adam(model.parameters(), lr=0.0001)

# 학습 루프: 모델을 10번의 epoch 동안 학습
num_epochs = 10
for epoch in range(num_epochs):
    model.train()  # 모델을 학습 모드로 설정
    optimizer.zero_grad()  # 옵티마이저의 기울기 초기화
    
    # 학습 데이터에서 입력과 타겟 설정 (여기서는 타겟과 입력이 동일하다고 가정)
    src_input = X_train.to(device)  # 학습 입력 시퀀스를 GPU로 이동
    tgt_input = X_train.to(device)  # 타겟도 동일하게 GPU로 이동
    
    # 모델을 사용해 예측 수행
    predictions = model(src_input, tgt_input)  # Softmax를 제거했으므로 squeeze() 필요 없음
    
    # 손실 계산: 예측과 실제 값을 비교해 손실 값 계산
    # CrossEntropyLoss는 클래스 레이블을 직접 받으므로 y_train의 차원을 맞춤
    loss = criterion(predictions, y_train.to(device))
    
    # 역전파를 통해 기울기 계산
    loss.backward()
    
    # 옵티마이저가 기울기를 사용해 모델 파라미터 업데이트
    optimizer.step()
...
# 테스트 데이터셋 평가
model.eval()  # 모델을 평가 모드로 전환 (dropout 등의 학습 관련 기능 비활성화)
with torch.no_grad():  # 평가 시에는 기울기를 계산하지 않음
    src_input = X_test.to(device)  # 테스트 입력 데이터를 GPU로 이동
    tgt_input = X_test.to(device)  # 테스트 타겟 데이터도 GPU로 이동
    
    # 테스트 데이터를 사용해 예측 수행
    test_preds = model(src_input, tgt_input)  # Softmax를 제거했으므로 squeeze() 필요 없음
    
    # 예측값에서 가장 확률이 높은 클래스를 선택 (다중 클래스 분류에서는 argmax 사용)
    test_preds = torch.argmax(test_preds, dim=1)
    
    # 정확도 계산: 예측값과 실제값을 비교해 정확도 계산
    accuracy = (test_preds == y_test.to(device)).float().mean()
