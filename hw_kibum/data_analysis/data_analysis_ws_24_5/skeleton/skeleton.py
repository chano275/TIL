...
# 참고: https://huggingface.co/docs/transformers//main_classes/trainer#transformers.Trainer.save_model
# 참고: https://huggingface.co/docs/transformers/main_classes/model#transformers.PreTrainedModel.save_pretrained
trainer.__________________("./results/gpt2-title-generator")
tokenizer.__________________("./results/gpt2-title-generator")

# 13. 추론을 위한 모델과 토크나이저 로드
# 저장된 모델과 토크나이저를 로드하여 추론에 사용합니다.
# 이는 훈련된 모델을 사용하여 새로운 데이터를 생성할 때 필요합니다.
model = GPT2LMHeadModel.__________________('./results/gpt2-title-generator')
tokenizer = PreTrainedTokenizerFast.__________________('./results/gpt2-title-generator')
...
def generate_title(input_text, max_length=32):
    ...
    # 생성된 텍스트 디코딩
    gen_text = tokenizer._______________(gen_ids[0], skip_special_tokens=True)
    return gen_text
